...
class Matrices {
  ...
  int getNumberOfSlots(List<List<Slot>> slots, {bool? validateDiagonal}) {
    int nxn = slots.length;
  ...
  List<List<Slot>> getRandomSlots(int range,
      {List<List<Slot>> slots = const []}) {
    final rng = Random();

    return List.generate(range, (j) {
      return List.generate(range, (i) {
        bool isActive = rng.nextBool();
        return Slot.fromMap({
          "color": isActive ? Colors.green[600] : Colors.blueAccent,
          "isActive": isActive,
          "controller": null,
          "coordenadas": {"i": i, "j": j}
        });
  ...
  // List<Widget> getWidgets(List<List<Slot>> slots, {required setList}) {
  //   List<Widget> list = [];
  ...
  //         controller: (controller) => _slot.animationController = controller,
  //         child: GestureDetector(
  //           onTap: () {
  //             _slot.animationController!.reset();
  //             _slot.animationController!.forward();
  //             _slot.isActive = !_slot.isActive!;
  //             _slot.color =
  //                 _slot.isActive! ? Colors.green[600] : Colors.blueAccent;
  //            setList();
  //           },
  //           child: Container(
  ...
  //                   topLeft: _slot.position!.i == 0 && _slot.position!.j == 0
  //                       ? const Radius.circular(30)
  //                       : const Radius.circular(10),
  //                   topRight: _slot.position!.i == slots.length - 1 &&
  //                           _slot.position!.j == 0
  //                       ? const Radius.circular(30)
  //                       : const Radius.circular(10),
  //                   bottomLeft: _slot.position!.i == 0 &&
  //                           _slot.position!.j == slots.length - 1
  //                       ? const Radius.circular(30)
  //                       : const Radius.circular(10),
  //                   bottomRight: _slot.position!.i == slots.length - 1 &&
  //                           _slot.position!.j == slots.length - 1
  //                       ? const Radius.circular(30)
  //                       : const Radius.circular(10)),
  //             ),
  ...
  List<List<Slot>> addSlots() {
    List<List<Slot>> list = [];
    return list;
  }

  List<List<Slot>> removeSlots() {
    List<List<Slot>> list = [];
    return list;
  }
}
